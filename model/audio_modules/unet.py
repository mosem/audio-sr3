...
class SelfAttention(nn.Module):
    ...
    def forward(self, input):
        ...
        qkv = self.qkv(norm).view(batch, n_head, head_dim * 3, time)
        query, key, value = qkv.chunk(3, dim=2)  # bhdt

        attn = torch.einsum(
            "bnct, bncs -> bnts", query, key
        ).contiguous() / math.sqrt(channel) # fix this for 1D
        attn = attn.view(batch, n_head, time, -1)
        attn = torch.softmax(attn, -1)
        attn = attn.view(batch, n_head, time, time)

        out = torch.einsum("bnts, bncs -> bnct", attn, value).contiguous()  # fix this for 1D
        out = self.out(out.view(batch, channel, time))
...
class UNet(nn.Module):
    def __init__(
        ...
        self.num_mults = len(channel_mults)
        pre_channel = inner_channel
        feat_channels = [pre_channel]
        now_res = image_size
        downs = [nn.Conv1d(in_channel, inner_channel,
                           kernel_size=3, padding=1)]
        for ind in range(self.num_mults):
            is_last = (ind == self.num_mults - 1)
        ...
        ups = []
        for ind in reversed(range(self.num_mults)):
            is_last = (ind < 1)
            use_attn = (now_res in attn_res)
            channel_mult = inner_channel * channel_mults[ind]
    ...
    def compute_output_length(self, length):
        for i in range(self.num_mults-1):
            length = math.floor((length-1)/2 + 1)
        for j in range(self.num_mults-1):
